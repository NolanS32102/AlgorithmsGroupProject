...
//             temp.pop();
//         }
//         // Stack to vector is target-to-start, so reverse it
//         reverse(rev.begin(), rev.end());

//         if (attemptNumber == 1) {
//             firstPath = rev;
//             bestPath = firstPath; // Default best is the first one
//         } else if (attemptNumber == 2) {
//             secondPath = rev;
//             // Compare and take the shorter path
//             if (secondPath.size() < firstPath.size() && !secondPath.empty()) {
//                 bestPath = secondPath;
//             }
//         }
        
//         solutionReady = true; // We now have at least one path to replay
//         replayIndex = 1;
//     }
...
//         // 1. DETECTION OF A NEW ATTEMPT (Reset occurred in main)
//         if (atStart(current) && !atStart(lastPos) && lastPos.x != -1) {
//             // Save what we learned from the run that JUST ended
//             if (!path.empty()) {
//                 saveCurrentPathAsSolution();
//             }
//             resetDfsState();
...
//         // 3. EXPLORATION MODE (Run 1 & 2)
//         if (path.empty()) {
//             path.push(current);
//             visited.emplace(current.x, current.y);
//         }

//         // Check for the target dynamically by peeking
//         for (DIRECTION dir : getDirections()) {
//             if (!car->look(dir)) {
//                 point next = nextPoint(current, dir);
//                 if (inBounds(next) && visited.count({next.x, next.y}) == 0) {
                    
//                     // Pre-check if this move is the goal
//                     // Note: Since Racer.h sets 'finished' inside move(), 
//                     // the easiest way is to let main move the car, then we catch 
//                     // it in the NEW RUN block above.
